...
function validateOrigin(req) {
    ...
    // If no allowed origins configured, allow all (fallback)
    if (ALLOWED_ORIGINS.length === 0) {
        return true;
    }
    
    // Check if origin matches allowed origins (supports wildcards and partial matches)
    const isValidOrigin = ALLOWED_ORIGINS.some(allowedOrigin => {
        ...
        // Starts with match (e.g., "https://60cod.github.io")
